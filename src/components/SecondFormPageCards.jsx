...
const SecondFormPageCards = ({ imgSrc, title, desc, isSelected, onSelect }) => {
  return (
    <div className="border-[1px] border-gray-300 w-[35%] rounded-xl">
      ...
      <div className="p-5">
        <p className="text-xl text-brand font-semibold">{title}</p>
        <p className="text-brand text-[14px] font-[380px] mt-[12px]">{desc}</p>

        <button
          className={`cursor-pointer w-full border-[1px] border-gray-400 shadow-xl font-medium py-3 px-6 rounded-lg mb-4 transition-colors mt-[40px] 
            ${
              isSelected === true
        ...
        <p className="flex justify-center text-brand items-center cursor-pointer">
          Learn more <ExternalLink size={13} />
        </p>
        <p className="flex justify-center mt-2 text-green-800 items-center">
